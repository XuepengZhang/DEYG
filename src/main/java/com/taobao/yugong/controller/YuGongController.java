...
import com.taobao.yugong.toCache.RecordCacheApplier;
import com.taobao.yugong.toCache.RecordCacheApplierImpl;
...
public class YuGongController extends AbstractYuGongLifeCycle {
    ...
    public void start() {
        ...
        String kafkaBroker = config.getString("yugong.kafka.broker");
        if (kafkaBroker == null || StringUtils.isEmpty(kafkaBroker)) {
            logger.error("kafka配置为空！");
...
//            String topic = tableHolder.table.getFullName();//TODO
            String topic = tableHolder.table.getName();//TODO
            RecordCacheApplier recordCache = new RecordCacheApplierImpl(kafkaBroker,topic);
            instance.setRecordCache(recordCache);
